	...
	#pragma omp parallel shared(a,b,n) private(tid)
	{
		...
		#pragma omp sections
		{
			#pragma omp section
			{
				...
				fprintf(stdout,"Hola, soy el hilo encargado del RectagleMethod %d\n",tid);
				rectangleMethod(b,a,n);
			}
			#pragma omp section
			{
				tid = omp_get_thread_num();
				fprintf(stdout,"Hola, soy el hilo encargado del midPointRule %d\n",tid);
				midPointRule(b,a,n);
			}
			#pragma omp section
			{	
				tid = omp_get_thread_num();
				fprintf(stdout,"Hola, soy el hilo encargado del trapezoidalMethod %d\n",tid);
				trapezoidalMethod(b,a,n);
			}	
			#pragma omp section
			{
				tid = omp_get_thread_num();
				fprintf(stdout,"Hola, soy el hilo encargado del simplsonRuleMethod %d\n",tid);
...
void rectangleMethod(double uplimit,double lowlimit,int interval){
	...
	sum = 0.0;
	//1 -> 4
	#pragma omp parallel shared(w,sum,interval,lowlimit,uplimit) private(tid)
	{
		tid = omp_get_thread_num();
		...
		for(z = 1;z <= interval;z++){
			...
			// fprintf(stdout,"(Hilo de Rectangle Method %d) sum = %lf\n",tid,sum);
		}
	}
...
void midPointRule(double uplimit,double lowlimit,int interval){
	...
	#pragma omp parallel shared(w,sum,uplimit,lowlimit,interval) private(tid)
	{
		tid = omp_get_thread_num();
		#pragma omp for schedule(static) nowait
		for (i = 0; i < interval; i++){
			z = lowlimit + (w/2) + i*w;
			sum = sum + f(z);
			// fprintf(stdout,"(Hilo de midPointRule %d) sum = %lf\n",tid,sum);
		}
	}
	sum = w * sum;
...
void trapezoidalMethod(double uplimit,double lowlimit,int interval){
	...
	#pragma omp parallel shared(w,sum,uplimit,lowlimit,interval) private(tid)
	{
		...
		for(z = 1;z <= interval;z++){
			x = lowlimit + z*w;
			sum = sum + 2 * f(x);
			// fprintf(stdout,"(Hilo de trapezoidalMethod %d) sum = %lf\n",tid,sum);
		}
	}
	sum = sum * w/2;
...
void simpsonRuleMethod(double uplimit,double lowlimit,int interval){
	...
	#pragma omp parallel shared(w,sum,uplimit,lowlimit,interval) private(tid)
	{
		...
		for (i = 1; i < interval; i++){
			...
			// fprintf(stdout,"(Hilo de simpsonRuleMethod %d) sum = %lf\n",tid,sum);
		}
	}

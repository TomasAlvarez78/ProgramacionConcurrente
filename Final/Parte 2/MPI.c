...
int main(int argc, char * argv[]){
	...
	if(rank == 0){
		MPI_Reduce(&sumRectangulo,&sumTotalRect,1,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
		fprintf(stdout,"Resultado total metodo del rectangulo: %.10f\n",sumTotalRect);
	}
	if(rank == 4){
		MPI_Reduce(&sumMidPoint,&sumTotalMid,1,MPI_DOUBLE,MPI_SUM,4,MPI_COMM_WORLD);
		fprintf(stdout,"Resultado total metodo del punto medio %.10f\n",sumTotalMid);
	}
	if(rank == 8){
		MPI_Reduce(&sumTrap,&sumTotalTrap,1,MPI_DOUBLE,MPI_SUM,8,MPI_COMM_WORLD);
		fprintf(stdout,"Resultado total metodo trapezoidal %.10f\n",sumTotalTrap);
	}
	if(rank == 12){
		MPI_Reduce(&sumSimp,&sumTotalSimp,1,MPI_DOUBLE,MPI_SUM,12,MPI_COMM_WORLD);
